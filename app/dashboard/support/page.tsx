...
export default function SupportPage() {
  ...
  const [tickets, setTickets] = React.useState([
    {
      id: "t1",
      subject: "Refund query for Booking BK-902145",
      priority: "MEDIUM",
      status: "CLOSED",
      date: "2026-06-25",
    },
    {
      id: "t2",
      subject: "KYC verification pending document issue",
      priority: "HIGH",
      status: "OPEN",
      date: "2026-06-29",
    },
      ...
      <div className="grid gap-8 md:grid-cols-3">
        ...
        <div className="space-y-6 rounded-xl border border-border bg-card/30 p-6">
          <h3 className="flex items-center gap-1.5 font-display text-base font-bold text-foreground">
            <LifeBuoy className="h-4 w-4" /> Active Tickets (
            {tickets.filter((t) => t.status === "OPEN").length})
          </h3>

          <div className="space-y-4">
            {tickets.map((t) => (
              <div key={t.id} className="space-y-1.5 border-b border-border/50 pb-3">
