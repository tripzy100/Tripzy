...
export default function CustomerAssistantPage() {
  ...
  const [messages, setMessages] = React.useState<Message[]>([
    {
      role: "assistant",
      content: "Hi! I am your Tripzy Copilot. Ask me about car rules, pickup hours, or extensions.",
    },
  ]);
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="flex h-[450px] flex-col rounded-2xl border border-border bg-card/30 p-6">
        {/* Messages body */}
        <div className="flex-1 space-y-4 overflow-y-auto pr-2">
          {messages.map((m, idx) => (
            <div
              key={idx}
              className={`flex max-w-[85%] gap-3 text-xs leading-relaxed ${
        ...
        {/* Input box */}
        <form onSubmit={sendMessage} className="mt-4 flex gap-2 border-t border-border/60 pt-4">
          <input
            value={input}
