    ...
    <div className="space-y-4">
      <form onSubmit={processImport} className="space-y-4">
        <div>
          ...
          <div className="text-[10px] text-muted-foreground mb-2 font-mono bg-muted p-2 rounded">
          ...
          <textarea
            ...
            rows={6}
            placeholder="KA-03-HA-1234,17CHARVINNUMBERMOC,Mahindra,Thar,LX,2024,Red,AUTOMATIC,PETROL,5000,Bengaluru,5,4,2500,5000,15"
            className="w-full rounded-lg border border-input bg-card px-3 py-2 text-xs font-mono text-foreground focus:outline-none"
          />
        </div>

        <Button type="submit" isLoading={loading} className="w-full">
          <Upload className="mr-1.5 h-4 w-4" /> Validate & Import Bulk Fleet
        </Button>
      </form>
      ...
      {logs.length > 0 && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 space-y-2">
          ...
          <div className="max-h-32 overflow-y-auto space-y-1 font-mono text-[10px] text-muted-foreground">
            {logs.map((log, idx) => (
              <div key={idx} className="border-b border-border/50 pb-1">{log}</div>
            ))}
          </div>
        </div>
      )}
