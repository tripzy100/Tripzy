...
import { softDeleteVehicle, updateVehicleStatus } from "../actions/cars-actions";
...
export function CarsDashboard({ initialVehicles }: { initialVehicles: DashboardVehicle[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card/30">
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-border text-sm text-foreground/80">
            {vehicles.map((car) => (
              <tr key={car.id} className="transition-colors hover:bg-muted/10">
                ...
                <td className="flex justify-end gap-2 p-4 text-right">
                  <Button
                    onClick={() =>
                      handleStatusChange(
                        car.id,
                        car.status === "AVAILABLE" ? "MAINTENANCE" : "AVAILABLE",
                        car.plateNumber,
                      )
                    }
                    variant="outline"
                  ...
                  <Button
                    onClick={() => handleDelete(car.id, car.plateNumber)}
                    variant="destructive"
                    size="sm"
                  >
      ...
      <Dialog isOpen={isCsvOpen} onClose={() => setIsCsvOpen(false)} title="Import Cars CSV">
        <CsvImporter
          onImportSuccess={() => {
            setIsCsvOpen(false);
            refreshList();
